...
var options = ["Bold,bold.png,b,b", "Italic,italic.png,i,i", "Underline,underline.png,u,u", "Strike,strike.png,s,s",];
...
commentarea.style.height = "187px";
makeIcons();

function makeIcons() {
	optionarea.innerHTML = "<div id='scratchformat'></div>" + optionarea.innerHTML;
	document.getElementById('scratchformat').innerHTML += "<div id='HelpPopup'><p onclick='"+ 'this.parentElement.style.display = "none";' +"' id='close1'>X</p><iframe src='https://pufflegamerz.github.io/ScratchFormat/documentation.html'>Whoops! Try upgrading your browser.</iframe></div>";
	...
	// Create an icon for custom options
	scratchformat.innerHTML += "<img src='https://pufflegamerz.github.io/ScratchFormat/assets2/questionmark.png' id='Help' title='Help' class='sf-option'>";
	scratchformat.innerHTML += "<img src='https://pufflegamerz.github.io/ScratchFormat/assets2/refresh.png' id='Refresh' title='Refresh' class='sf-option'>";
	...
	scratchformat.innerHTML += "<img src='https://pufflegamerz.github.io/ScratchFormat/assets2/link.png' id='Link' title='Link' class='sf-option'>";
	format();
}
...
function format() {
	for (var n = 0; n < document.getElementsByClassName('content').length; n++) {
		...
		comment = comment.replace(/\(link=([^\n)]*)\)/g,"<a target='_newtab' href='$1'>");
		comment = comment.replace(/\(\/link\)/g,"</a>");
...
document.getElementById("Link").onclick = function() {
	var textarea = document.getElementsByClassName('control-group tooltip right')[0].getElementsByTagName('textarea')[0];
	var sel = textarea.value.substring(textarea.selectionStart, textarea.selectionEnd);
	var finText = textarea.value.substring(0, textarea.selectionStart) + '(link=https:https://www.google.com)' + sel + '(/link)' + textarea.value.substring(textarea.selectionEnd);
	textarea.value = finText;
	textarea.focus();
	textarea.selectionEnd = textarea.selectionEnd + 1 + t1.length;
}
